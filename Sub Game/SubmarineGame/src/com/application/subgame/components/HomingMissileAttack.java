...
public class HomingMissileAttack implements Attack{
	...
	public void attack(){
		...
		for (int i = 0; i< 10;i++){
			...
			if (list.size() == 0){
				game.getMap().addMissiles(new HomingMissile(player.getX() + 36 + ranx,player.getY() - 30 + rany,this.dmg,this.var, BitmapFactory.decodeResource(game.getResource(), R.drawable.homingmissile),null,this.game));
				...
				game.getMap().addMissiles(new HomingMissile(player.getX() + 36 + ranx,player.getY() - 30 + rany,this.dmg,this.var, BitmapFactory.decodeResource(game.getResource(), R.drawable.homingmissile),target,this.game));
				game.getMap().addStuff(new Bubble(player.getX() + 22 + ranx,player.getY() - 20 + rany,BitmapFactory.decodeResource(game.getResource(), R.drawable.bubble)));
